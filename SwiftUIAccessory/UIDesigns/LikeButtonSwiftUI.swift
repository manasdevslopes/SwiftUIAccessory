...
struct LikeButtonSwiftUI: View {
    var body: some View {
        ZStack{
            Color.black.edgesIgnoringSafeArea(.all)
            LikeButtonView().offset(x: 0, y: -20)
        }
    }
}
...
struct LikeButtonView: View {
    var body: some View {
        ZStack {
            ...
            VStack {
                Spacer()
                    .frame(width: 0, height: 700, alignment: .center)
                Text("@_iosmanas")
                    .font(.title)
                    .fontWeight(.light)
            ...
            ZStack {
                ...
                if !self.finish {
                    Circle()
                        ...
                        .animation(Animation.easeInOut(duration: touch ? 0.5 : 0).delay(0.5), value: touch)
                ...
                ZStack {
                    ...
                    Image("hand.thumbsup.fill")
                        ...
                        .animation(Animation.timingCurve(0.17,1.67,0.61,0.77, duration: touch ? 0.8 : 0).delay(touch ? 0.8 : 0), value: touch)
                    ...
                    Image(systemName: "hand.thumbsup.fill")
                        ...
                        .animation(Animation.timingCurve(0.54, 1.65, 0.68, 0.79, duration: touch ? 1 : 0).delay(touch ? 2.5 : 0), value: touch)
                }.frame(width: 80)
            }.frame(width: 80)
        }
        .frame(width: 80)
        ...
        ZStack {
            ForEach(positions, id: \.self) { pos in
                Circle()
                    ...
                    .animation(Animation.timingCurve(0.5, 1, 0.89, 1, duration: touch ? pos.duration : 0).delay(touch ? 0.3 : 0), value: touch)
                    .opacity(touch ? 0.4 : 1)
                    .hueRotation(Angle(degrees: touch ? pos.color + 360 : 0))
                    .animation(Animation.timingCurve(0.5, 1, 0.89, 1, duration: touch ? 1 : 0).delay(touch ? 1.5 : 0), value: touch)
                    .scaleEffect(x: touch ? 0 : 1, y: touch ? 0 : 1, anchor: .center)
                    .offset(x: touch ? pos.x2 : 0, y: touch ? pos.y2 : 0)
                    .animation(Animation.timingCurve(0.5, 1, 0.89, 1, duration: touch ? 1.8 : 0).delay(touch ? pos.delay : 0), value: touch)
            }
        }.frame(width: 20, height: 20)
    }
}

struct LikeButtonSwiftUI_Previews: PreviewProvider {
